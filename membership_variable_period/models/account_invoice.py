...
class AccountInvoiceLine(models.Model):
    ...
    def _prepare_membership_line(self, invoice, product, price_unit, line_id, qty=1.0):
        qty = int(math.ceil(qty))
        date_from = fields.Date.from_string(invoice.date_invoice or fields.Date.today())
        date_to = product.product_tmpl_id._get_next_date(
            date_from, qty=qty
        ) - timedelta(days=1)
        return {
    ...
    def create(self, vals):
        ...
        if (
            ...
            and line.product_id.membership_type == "variable"
        ):
