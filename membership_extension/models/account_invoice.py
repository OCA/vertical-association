...
class AccountInvoice(models.Model):
    ...
    def action_invoice_draft(self):
        self.mapped("invoice_line_ids.membership_lines").write(
            {"date_cancel": False, "state": "waiting",}
        )
    ...
    def action_cancel(self):
        ...
        refunds = self.filtered(
            lambda r: (r.type == "out_refund" and r.refund_invoice_id)
        )
    ...
    def invoice_validate(self):
        ...
        for refund in self.filtered(lambda r: r.type == "out_refund"):
            origin = self.search(
                [("type", "=", "out_invoice"), ("number", "=", refund.origin),]
            )
            lines = origin.mapped("invoice_line_ids.membership_lines")
            if origin and lines:
                if origin.amount_untaxed == refund.amount_untaxed:
                    lines.write(
                        {"state": "canceled", "date_cancel": refund.date_invoice,}
                    )
                else:
                    lines.write(
